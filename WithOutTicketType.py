...
class PredictionData:
    ...
    { '_id': 0, 'customer_id': 1, 'start_date': 1, 'end_date': 1, 'influencer_type': 1 })
    ...
    def get_prediction_data(self, start_date,end_date,customerid):
        ...
        pipeline_2 = [
            ...
            {'$group': {'_id': {'date': '$date.date', 'customer_id': '$customer_id'},
...
if __name__ == "__main__":
    ...
    for i, j in d.iterrows():
        ...
        for b in range(len(list_influencer)):
            if d.loc[i, "date"] >= list_influencer[b]["start_date"] and d.loc[i, "date"] <= list_influencer[b]["end_date"]:
...
d.to_csv(f"{filename}.csv",
                  columns=['date', 'customer_id', 'sold_qty', 'scan_qty','avgtemp_f',
