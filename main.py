...
if __name__ == "__main__":
    ...
    df3=df2.copy()
    df3 = df3.groupby(['customer_id', 'Year-Month']).agg({'temperature_celsius':'mean','temperature_farenheit':'mean','sold_qty':'sum','scan_qty':'sum'}
       ).reset_index()

csv_export = df3.to_csv(sep=",", quoting=QUOTE_ALL, quotechar='"', columns=[ 'customer_id','Year-Month', 'sold_qty', 'scan_qty','temperature_celsius','temperature_farenheit'])
print("\n CSV data:", csv_export)

df3.to_csv(f"exportmonth.csv",
                  columns=['customer_id','Year-Month', 'sold_qty', 'scan_qty','temperature_celsius','temperature_farenheit'],
